...
import pkginfo  # type: ignore[import]
...
ldap_config = pyproject.get("tool", {}).get("python-ldap", {}).get("config", {})
for name in dir(LDAP_CLASS):
    if name in ldap_config:
        setattr(LDAP_CLASS, name, ldap_config[name])
print("Using build configuration from pyproject.toml")
...
setup(
    ext_modules=[
        Extension(
            ...
            runtime_library_dirs=(not sys.platform.startswith("win"))
            * LDAP_CLASS.library_dirs,
            define_macros=LDAP_CLASS.defines
            + (
                "sasl" in LDAP_CLASS.libs
                or "sasl2" in LDAP_CLASS.libs
                or "libsasl" in LDAP_CLASS.libs
            )
            * [("HAVE_SASL", None)]
            + ("ssl" in LDAP_CLASS.libs and "crypto" in LDAP_CLASS.libs)
            * [("HAVE_TLS", None)]
            + [
