...
import ldap.cidict
from ldap.schema.tokenizer import extract_tokens, split_tokens
...
class SchemaElement:
    ...
    def key_attr(self, key, value, quoted=0):
        assert value is None or isinstance(value, str), TypeError(
            f"value has to be of str, was {value!r}"
        )
        if value:
            if quoted:
    ...
    def key_list(self, key, values, sep=" ", quoted=0):
        assert isinstance(values, tuple), TypeError(
            f"values has to be a tuple, was {values!r}"
        )
        if not values:
            return ""
        if quoted:
            quoted_values = [
                "'{}'".format(value.replace("'", "\\'")) for value in values
            ]
        else:
...
class AttributeType(SchemaElement):
    ...
    def _set_attrs(self, tokens, d):
        ...
        self.single_value = d["SINGLE-VALUE"] is not None
        self.collective = d["COLLECTIVE"] is not None
        self.no_user_mod = d["NO-USER-MODIFICATION"] is not None
    ...
    def __str__(self):
        ...
        if self.syntax_len is not None:
            result.append(f"{{{self.syntax_len}}}")
