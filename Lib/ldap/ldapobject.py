...
class SimpleLDAPObject:
    ...
    CLASSATTR_OPTION_MAPPING: ClassVar = {
    ...
    def _ldap_call(self, func, *args, **kwargs):
        ...
        try:
            try:
                ...
                if (
                    __debug__
                    and self._trace_level >= 2
                    and func.__name__ != "unbind_ext"
                ):
            ...
            if __debug__ and self._trace_level >= 2:
                self._trace_file.write(
                    f"=> LDAPError - {e.__class__.__name__}: {e!s}\n"
                )
            raise
        else:
            if __debug__ and self._trace_level >= 2:
                if diagnostic_message_success is not None:
                    self._trace_file.write(
                        f"=> diagnosticMessage: {diagnostic_message_success!r}\n"
                    )
    ...
    def bind(self, who, cred, method=ldap.AUTH_SIMPLE):
        ...
        """
        assert method == ldap.AUTH_SIMPLE, (
    ...
    def extop_result(self, msgid=ldap.RES_ANY, all=1, timeout=None):
        _resulttype, _msg, msgid, _respctrls, respoid, respvalue = self.result4(
            msgid,
            all=1,
    ...
    def result2(self, msgid=ldap.RES_ANY, all=1, timeout=None):
        resp_type, resp_data, resp_msgid, _resp_ctrls = self.result3(
            msgid, all, timeout
        )
        return resp_type, resp_data, resp_msgid

    def result3(self, msgid=ldap.RES_ANY, all=1, timeout=None, resp_ctrl_classes=None):
        resp_type, resp_data, resp_msgid, decoded_resp_ctrls, _retoid, _retval = (
            self.result4(
                msgid,
                all,
                timeout,
    ...
    def reconnect(self, uri, retry_max=1, retry_delay=60.0, force=True):
        ...
        try:
            ...
            while reconnect_counter:
                counter_text = f"{retry_max - reconnect_counter + 1}. (of {retry_max})"
                if __debug__ and self._trace_level >= 1:
                    self._trace_file.write(
                        f"*** Trying {counter_text} reconnect to {uri}...\n"
                    )
                try:
                    try:
                        # Do the connect
                    ...
                    if __debug__ and self._trace_level >= 1:
                        self._trace_file.write(
                            f"*** {counter_text} reconnect to {uri} failed\n"
                        )
