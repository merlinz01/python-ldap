...
ldif_pattern = "^((dn(:|::) {dn_pattern})|({attrtype_pattern}(:|::) .*)$)+".format(
    **vars()
)
...
class LDIFWriter:
    ...
    def unparse(self, dn, record):
        ...
        if isinstance(record, dict):
            ...
            raise ValueError(
                f"Argument record must be dictionary or list instead of {record!r}"
            )
...
class LDIFParser:
    ...
    def _next_key_and_value(self):
        ...
        try:
            ...
            raise ValueError(f"no value-spec in {unfolded_line!r}") from ve
    ...
    def parse_change_records(self):
        ...
        while k is not None and (
            ...
            if k == "changetype":
                # v is still bytes, spec says it should be valid utf-8; decode it.
                v = v.decode("utf-8")
                if v not in valid_changetype_dict:
