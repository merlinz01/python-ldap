...
HAVE_LDAPI = False if "LDAPI" in CI_DISABLED else hasattr(socket, "AF_UNIX")
...
class SlapdObject:
    ...
    def __init__(self):
        ...
        self.testrundir = os.path.join(self.TMPDIR, f"python-ldap-test-{self._port}")
        self._slapd_conf = os.path.join(self.testrundir, "slapd.d")
        self._db_directory = os.path.join(self.testrundir, "openldap-data")
        self.ldap_uri = f"ldap://{self.local_host}:{self._port}/"
        if HAVE_LDAPI:
