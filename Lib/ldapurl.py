...
class LDAPUrl:
    ...
    attr2extype: ClassVar = {"who": "bindname", "cred": "X-BINDPW"}
    ...
    def _parse(self, ldap_url):
        ...
        if paramlist_len >= 3:
            ...
            try:
                ...
                raise ValueError(f"Invalid search scope {scope!r}") from e
        if paramlist_len >= 4:
    ...
    def unparse(self):
        ...
        """
        attrs_str = "" if self.attrs is None else ",".join(self.attrs)
        scope_str = SEARCH_SCOPE_STR[self.scope]
        filterstr = "" if self.filterstr is None else ldapUrlEscape(self.filterstr)
        ...
        if self.urlscheme == "ldapi":
            ...
            hostport = self.hostport
        ldap_url = (
            f"{self.urlscheme}://{hostport}/{dn}?{attrs_str}?{scope_str}?{filterstr}"
        )
        if self.extensions:
    ...
    def htmlHREF(self, urlPrefix="", hrefText=None, hrefTarget=None):
        ...
        return f'<a{target} href="{urlPrefix}{self.unparse()}">{hrefText}</a>'
