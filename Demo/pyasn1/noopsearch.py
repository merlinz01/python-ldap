...
try:
    ...
    _, _, _, search_response_ctrls = ldap_conn.result3(
        msg_id, all=1, timeout=SEARCH_TIMEOUT
    )
except (
    ldap.TIMEOUT,
...
noop_srch_ctrl = next(
    c for c in search_response_ctrls if c.controlType == SearchNoOpControl.controlType
)

print("Number of search results:", noop_srch_ctrl.numSearchResults)
print("Number of search continuations:", noop_srch_ctrl.numSearchContinuations)
