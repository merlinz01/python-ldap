...
class DeleteLeafs(ldap.asyncsearch.AsyncSearchHandler):
    ...
    def startSearch(self, searchRoot, searchScope):
        if searchScope not in [ldap.SCOPE_ONELEVEL, ldap.SCOPE_SUBTREE]:
            raise ValueError(
                "Parameter searchScope must be either ldap.SCOPE_ONELEVEL or ldap.SCOPE_SUBTREE."
            )
        self.nonLeafEntries = []
        self.deletedEntries = 0
    ...
    def _processSingleResult(self, resultType, resultItem):
        if resultType in self._entryResultTypes:
            ...
            hasSubordinates = entry.get(
                "hasSubordinates", entry.get("hassubordinates", ["FALSE"])
            )[0]
            numSubordinates = entry.get(
                "numSubordinates", entry.get("numsubordinates", ["0"])
            )[0]
